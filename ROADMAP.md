...
Current milestone: Milestone 2 - Native Brain (started; `BRN-001`, `BRN-002`, `BRN-004`, `BRN-005`, and `BRN-006` done, `BRN-008`'s run controller and repository test doubles partially done, `BRN-003` gated on evidence, `BRN-007`+ next. Milestone 1 is complete apart from `OWN-001` through `OWN-005`, which are owner-gated)
