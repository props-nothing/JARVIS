...
| `R-025` | Voice research exists but is excluded from the repository and the docs gate, so it cannot be reviewed or cited | Medium | High | Promote durable findings into evidence notes; keep only runnable harnesses out of the repository | Research/architecture |
